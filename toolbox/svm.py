...
class SVC:
    ...
    def train(self, X, Yin, C, kernelFunction, tol = 1e-3, max_passes = 5):
        """
    ...
    def predict(self, Xin, precompute=False):
        """
        ...
        if precompute:
            K = self.kernelFunction( X, self.X )
            K = self.y * self.alphas * K
            p = np.sum(K, axis=1) + self.b
        else:
            p = np.zeros(m)
            ...
            for i in range(m):
                ...
                for j in range(self.X.shape[0]):
                    ...
                    prediction = prediction + \
                        self.alphas[j] * self.y[j] * \
                        self.kernelFunction(xi, xj)
